...
Флаги важны: `ruff` настроен с `fix = true`, поэтому без `--no-fix` он правит файлы вместо проверки, а `bandit` без `-c pyproject.toml` не видит секцию `skips` и падает на собственном коде проекта. По той же причине `just lint` не только проверяет, но и форматирует дерево: рецепты `ruff` и `black` внутри него запускаются без `--check`. Перед PR это удобно, но если нужно именно увидеть, что найдёт CI, ничего не меняя, - берите команды выше.
