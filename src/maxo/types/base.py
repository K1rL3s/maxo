...
class BaseMethodsFacade:
    ...
    От него растёт вся цепочка фасадов (`AttachmentsFacade` ->
    `ChatMethodsFacade` -> ..., плюс `CallbackMethodsFacade`). Он же подмешан в
    `MaxoType`, поэтому любой тип несёт `bot`/`as_` (это наследование со временем
    уберём - бот на типе нужен не всем).
