...
``ErrorMiddleware`` оборачивает только обработку обновлений. Исключения из обработчиков сигналов (``before_startup``, ``after_startup``, ``before_shutdown``, ``after_shutdown``) сюда не попадают и доходят до кода, который запустил бота, - подробнее в :doc:`signals`. Исключение из этого правила - ``before_shutdown``/``after_shutdown`` в long polling: там ошибка обработчика логируется и гасится, а не доходит до вызывающего кода, - см. :doc:`signals` и :doc:`long-polling`.
