...
Старый вариант ``MagicFilter(F.text == "hello")`` сохраняется, но выдаёт
``DeprecationWarning``. Передавайте выражение ``F`` напрямую.
...
Дополнительные именованные параметры фильтра автоматически берутся из ``ctx``.
Дополнительные именованные параметры фильтра автоматически берутся из ``ctx``.
