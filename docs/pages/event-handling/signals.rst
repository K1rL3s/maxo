...
Исключение из этого правила - ``BeforeShutdown``/``AfterShutdown`` в
long polling (:doc:`long-polling`): там сигналы всегда шлются из ``finally``,
чтобы гарантировать их парность на любом раннем выходе, в том числе на
``task.cancel()``. Упавший там обработчик залогируется и будет проигнорирован,
а не заменит собой исходную причину остановки (например, ``CancelledError``) -
подробности в :doc:`long-polling`. В вебхуках (``SimpleWebhookEngine.on_shutdown``)
это правило не действует: ошибка обработчика по-прежнему пробрасывается как
описано выше.
