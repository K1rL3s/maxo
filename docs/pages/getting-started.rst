...
- ``'maxo[fastapi]'`` - для вебхуков на ``FastAPI`` (для ``aiohttp`` ничего доставлять не нужно)
- ``'maxo[preview]'`` - для PNG-диаграммы переходов диалогов; дополнительно нужен системный ``graphviz``
